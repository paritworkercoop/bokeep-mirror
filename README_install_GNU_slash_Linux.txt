...
If you've installed it to someplace other than /usr/ or /usr/local, you'll need to set PYTHONPATH to include it
$ export PYTHONPATH=/path/to/zodb_install/lib/python2.x/site-packages/:$PYTHONPATH
...
# mkdir -p /opt/src/
# cd /opt/src/
# wget http://downloads.sourceforge.net/sourceforge/gnucash/gnucash-2.4.0.tar.bz2
# tar xjf gnucash-2.4.0.tar.bz2
# cd gnucash-2.4.0
# ./configure --prefix=/opt/gnucash-2.4.0/ --enable-python-bindings
# make && make install

To use the python bindings, you need to either install GnuCash in a place like /usr/ or /usr/local where your python interpreter can find it, or set the PYTHONPATH environmental variable
...
Bo-keep can be installed with its setup.py script like so
# python setup.py install
or
# python setup.py install --prefix=/path/to/where/you/want/bokeep/

We're make a python egg available in PyPi and provide a debian package to make this even easier eventually.
